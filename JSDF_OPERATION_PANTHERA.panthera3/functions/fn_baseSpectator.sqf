...
["toggleSpectator", {
    ...
    if (_thisArgs getVariable [QGVAR(baseSpectatorAllowed), false]) then {
        ...
        ["ace_common_systemChatGlobal", "[Admin] Spectator Prohibited"] call CBA_fnc_globalEvent;
    } else {
        _thisArgs setVariable [QGVAR(baseSpectatorAllowed), true, true];
        ["ace_common_systemChatGlobal", "[Admin] Spectator Allowed"] call CBA_fnc_globalEvent;
    };
